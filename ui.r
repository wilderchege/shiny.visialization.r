...
shinyUI(
  fluidPage(
    ...
    fluidRow(
      column(12, 
             wellPanel(selectInput("country", "country_list", choices = c("United-States", "Canada", "Mexico", "Germany", "Philippines")))  # add select input 
             )
    ),
    ...
    fluidRow(
      column(3, 
             wellPanel(
               ...
               checkboxInput("is_stacked", "stacked bar", value = TRUE)     # add check box input for stacked bar chart option
               )
             ),
